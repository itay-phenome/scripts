...
R_S3_BUCKET="s3://unity-repos-for-r-packages/R_VERSION_4_1_2"
...
r_lib() {
    ...
    aws s3 cp ${R_S3_BUCKET} /root/R_packages --recursive

    cat <<EOF | tee ./install.R >/dev/null
#!/usr/bin/env Rscript

local_dir <- '/root/R_packages'
package_files <- list.files(local_dir, pattern = '\\\\.tar\\\\.gz$', full.names = TRUE)
install_packages <- function(pkg_file) {
install.packages(pkg_file, repos = NULL, type = 'source')
}
sapply(package_files, install_packages)

EOF
    Rscript install.R

    # Confirm the installation
    echo "R packages installed from /root/R_packages"
...
export REVISION="17535"
export CHANGELOG="1234"
export MYSQL_HOST="192.168.1.26"
export MYSQL_USER="phenome"
export MYSQL_PASS="6DTJu2lDS6sp"
export LOCAL_STORAGE="/data/public/"
export WS_ROOT="https://danziger.phenome-networks.com/ /public"
export SQS="danziger-on-prem"
